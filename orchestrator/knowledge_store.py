...
class KnowledgeStore:
    ...
    def _init_tables(self):
        ...
        # 기존 DB 마이그레이션: provider 컬럼이 없으면 추가
        try:
            cur.execute("SELECT provider FROM agents LIMIT 1")
        except sqlite3.OperationalError:
            cur.execute("ALTER TABLE agents ADD COLUMN provider TEXT DEFAULT 'claude-code'")
        ...
        with self._lock:
            self.conn.execute(
                """INSERT INTO agents (name, url, description, skills, data_paths, mcp_servers,
                                       agent_type, alias, provider, last_seen, created_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                (
                    ...
                    agent_type, alias, provider, now, now,
                ),
            )
            self.conn.commit()
    ...
    def load_agents(self) -> List[Dict]:
        with self._lock:
            ...
            for row in rows:
                ...
                d.setdefault("provider", "claude-code")
                result.append(d)
            return result
